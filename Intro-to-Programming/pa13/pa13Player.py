...
class Player(object):
    ...
    def set_position(self, pX=0.0, pY=0.0):
        ...
        if self.__health > 0 :
            ...
            print("The player's health is at 0 or below")
            #return without changing the Player's position
            x1 = self.x - self.__radius
            y1 = self.y - self.__radius
            x2 = self.x + self.__radius
            y2 = self.y + self.__radius
    ...
    def got_hit(self):
        """when gets hit, decremented by 1 in health

        Player -> int """

        self.__health = self.__health -1

        return self.__health #returns the health value



    def get_health(self):
        """returns the health value of player
